...
export ZAN_HOME_NAME=$(jq --raw-output '.ZAN_HOME_NAME // "Dům Žán"' /data/options.json)
export CHAT_NAME_ONDRA=$(jq --raw-output '.CHAT_NAME_ONDRA // "Ondra"' /data/options.json)
export CHAT_NAME_JANA=$(jq --raw-output '.CHAT_NAME_JANA // "Jana"' /data/options.json)
